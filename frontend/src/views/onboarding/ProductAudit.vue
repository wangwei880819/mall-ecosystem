<template>
  <div class="page-container">
    ...
    <!-- 审核对话框 -->
    <el-dialog v-model="showAuditDialog" title="商品审核" width="900px" :close-on-click-modal="false" @closed="resetAudit">
    ...
    <el-dialog v-model="showApproveReasonDialog" title="审核通过原因" width="500px" append-to-body>
      ...
      <el-form label-position="top">
        <el-form-item label="审核通过原因（必填）">
          <el-input v-model="approveReason" type="textarea" :rows="4" placeholder="请填写人工审核通过的原因，如：商品信息已核实无误、商品具备特殊资质等" />
        </el-form-item>
      </el-form>
      <template #footer>
...
const fetchProducts = async () => {
  loading.value = true
  try {
    const res = await request.get('/product/audit-list')
    if (res.code === 200) {
...
const approveProduct = async () => {
  ...
  try {
    await ElMessageBox.confirm('确认审核通过该商品？通过后商品将立即上架。', '审核确认', {
...
.ai-score-circle.passed {
  border-color: #67c23a;
  background: #f0f9eb;
}

.ai-score-circle.failed {
  border-color: #f56c6c;
  background: #fef0f0;
}
